...
class TestPasswordResetAPI(TestOrganizationMixin, TestCase):
    ...
    def test_reset_request_user_without_email_does_not_receive_email(self):
        cases = (
            ("username", "no-email"),
            ("phone number", "+12025551234"),
        )
        self._create_user(username="no-email", email=None, phone_number="+12025551234")
        for identifier_type, identifier in cases:
            with self.subTest(identifier_type=identifier_type):
                response = self.client.post(self.request_url, {"input": identifier})
                self.assertEqual(response.status_code, 200)
                self.assertEqual(len(mail.outbox), 0)
