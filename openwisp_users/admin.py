...
class UserAdmin(MultitenantAdminMixin, BaseUserAdmin, BaseAdmin):
    ...
    @admin.action(
        permissions=['change'], description=_('Flag selected users as inactive')
    )
    ...
    @admin.action(
        permissions=['change'], description=_('Flag selected users as active')
    )
    @require_confirmation
    ...
    @admin.action(permissions=['delete'], description=delete_selected.short_description)
